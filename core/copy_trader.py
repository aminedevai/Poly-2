"""core/copy_trader.py — Mirrors a target wallet's trades at a configurable scale."""
import json, os, time
...
class CopyTrader:
    def __init__(self, memory: dict):
        ...
        for d in memory.get("closed_trades", []):
            try:
                self.closed_trades.append(ClosedTrade.from_dict(d))
            except:
                pass

    def load_existing(self) -> tuple:
        """
        On startup: fetch the target wallet's current positions and
        load them into our tracker immediately (so we don't miss them).
        Returns (n_total, n_loaded).
        """
        ...
        for key, r in raw.items():
            ...
            pos = Position(
                ...
                shares=our_shares,
                entry_amount=needed,
                cur_value=r["cur_price"] * our_shares,
                end_date=r["end_date"],
            )
            self.positions[key] = pos
            copied += 1
        self._prev_target = raw
        log.info(f"Startup: loaded {copied}/{len(raw)} target positions")
    ...
    def sync(self) -> List[tuple]:
        """
        Poll the target wallet, compare to previous snapshot.
        Returns a list of (kind, message) tuples for display.
        Kinds: "new", "close", "add", "skip"
        """
        events  = []
        ...
        for key, raw in current.items():
            if key in self._prev_target:
                # Position already existed — check if target added shares
                ...
                if added > 0.01 and key in self.positions:
                    ...
                    if extra <= self.available:
                        ...
                        events.append(("add",
                            f"ADD  {raw['market_title'][:40]}  +{added * self.scale:.1f} shares"))
                continue

            # Brand-new position opened by target
            needed = raw["avg_price"] * raw["shares"] * self.scale
            if needed < 0.01:
                continue
            if needed > self.available:
                events.append(("skip",
                    f"SKIP  {raw['market_title'][:35]}  — need ${needed:.2f}"))
                continue
            ...
            pos = Position(
                ...
                entry_price=raw["avg_price"],
                cur_price=raw["cur_price"],
                shares=our_shares,
                entry_amount=needed,
                cur_value=raw["cur_price"] * our_shares,
            ...
            events.append(("new",
                ...
                f"         https://polymarket.com/event/{raw['slug']}"
            ))
            log.info(f"COPIED {raw['market_title'][:40]}  {raw['outcome']}  ${needed:.2f}")

        # Update prices on existing positions; close expired or target-closed ones
        ...
        # Manual close requests (typed by user in terminal)
        for key in list(self.manual_queue):
            if key in self.positions:
                ct = self._do_close(key, reason="manual")
                if ct:
                    events.append(("close",
                        f"MANUAL  {ct.market_title[:40]}  P&L=${ct.realized_pnl:+.2f}"))
    ...
    def close_all(self, reason: str = "shutdown") -> List[ClosedTrade]:
        return [ct for key in list(self.positions)
                if (ct := self._do_close(key, reason))]
