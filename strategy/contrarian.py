"""strategy/contrarian.py — Volume Contrarian: bet UP on high-vol markets at T-30s."""
import csv, os, time
...
class Contrarian:
    ...
    def __init__(self, budget=100.0, bet_size=10.0, min_volume=5000.0):
        ...
        self.closed_trades: List[ContrarianTrade] = []
        self._fired: set = set()
    ...
    def on_snapshot(self, slug, up_price, volume, seconds_before):
        ...
        t = ContrarianTrade(slug=slug, direction="UP", entry_price=entry_price,
                            bet_size=self.bet_size, volume=volume)
        self.open_trades[slug] = t; self._fired.add(slug)
        log.info(f"CONTRARIAN ENTER {slug[-10:]} UP @ {entry_price:.3f} vol=${volume:,.0f}")
