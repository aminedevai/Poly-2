"""strategy/contrarian.py — Vol Contrarian: bet UP in $1k-$20k volume. Entry: UP token $0.40-$0.4999."""
import csv, os, time
...
class Contrarian:
    ...
    def __init__(self, budget=100.0, bet_size=5.0, min_volume=1000.0, max_volume=20000.0,
        ...
        self.up_min = up_min; self.up_max = up_max
        self.open_trades: Dict[str, ContrarianTrade] = {}
        self.closed_trades: List[dict] = []
        self._csv = os.path.join("logs", "contrarian_trades.csv")
        os.makedirs("logs", exist_ok=True)
        self._ensure_csv()
        self._fired: set = load_fired_from_csv(self._csv)
        self._load_history()
        log.info(f"CONTRARIAN init: {len(self.closed_trades)} closed trades loaded")
            ...
            with open(self._csv, encoding='utf-8') as f:
                for row in csv.DictReader(f):
                    ...
                    self.closed_trades.append({
                        'slug': row['slug'], 'direction': row.get('direction', 'UP'),
    ...
    def _save(self, t: ContrarianTrade):
    ...
    def on_snapshot(self, slug, up_price, volume, seconds_before):
        ...
        if volume < self.min_volume:
            log.info(f"CONTRARIAN SKIP {slug[-10:]} vol=${volume:,.0f} < ${self.min_volume:,.0f}"); return
        if volume > self.max_volume:
            log.info(f"CONTRARIAN SKIP {slug[-10:]} vol=${volume:,.0f} > ${self.max_volume:,.0f}"); return
        if self.capital < self.bet_size: return
        if not (self.up_min <= up_price <= self.up_max):
            log.info(f"CONTRARIAN SKIP {slug[-10:]} up={up_price:.4f} not in [{self.up_min},{self.up_max}]"); return
        self.capital -= self.bet_size
        t = ContrarianTrade(slug=slug, direction="UP", entry_price=up_price, bet_size=self.bet_size, volume=volume)
        self.open_trades[slug] = t; self._fired.add(slug)
        log.info(f"CONTRARIAN ENTER ✓ {slug[-10:]} UP @ {up_price:.4f}  vol=${volume:,.0f}")
    ...
    def on_outcome(self, slug, outcome):
        ...
        self._save(t)
        log.info(f"CONTRARIAN SETTLE {slug[-10:]} → {outcome} {'WIN ✓' if won else 'LOSS ✗'} {t.profit:+.2f}")
    ...
    def summary(self):
        ...
        return {
            ...
            'open_trades': [
                {'slug': t.slug, 'direction': 'UP', 'entry_price': round(t.entry_price, 4),
