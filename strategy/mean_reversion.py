"""strategy/mean_reversion.py — Fade late price deviations. Hold to settlement."""
import csv, os
    ...
    def on_snapshot(self, slug: str, up_price: float, volume: float, seconds_before: int):
        ...
        if seconds_before == 30:
            ...
            if deviation < trigger:
                log.info(f"MR SKIP dev={deviation:.3f} < {trigger}"); return
            if self.capital < bet_size:
                log.info(f"MR SKIP capital={self.capital:.2f}"); return

            direction   = "DOWN" if up_price > 0.50 else "UP"
            entry_price = (1.0 - up_price) if up_price > 0.50 else up_price

            self.capital -= bet_size
            trade = MRTrade(slug=slug, direction=direction,
        ...
        # ── Record p10 for analysis, but HOLD to settlement ───────────────────
            ...
            log.info(
                ...
                f"p10={up_price:.3f}  unrealized=${unreal:+.2f}  → holding"
            )

    def on_outcome(self, slug: str, outcome: str):
        """Market resolved — settle at $1.00 (win) or $0.00 (loss)."""
        if slug not in self.open_trades: return
        ...
        t.profit     = (shares * 1.0 - MR['bet_size']) if won else -MR['bet_size']
