"""strategy/always_down.py — Always Bet DOWN baseline: fires on every market at T-30s."""
import csv, os, time
...
class AlwaysDown:
    ...
    def __init__(self, budget=100.0, bet_size=10.0, min_volume=100.0):
        ...
        self.closed_trades: List[AlwaysDownTrade] = []
        self._fired: set = set()
    ...
    def on_snapshot(self, slug, up_price, volume, seconds_before):
        ...
        t = AlwaysDownTrade(slug=slug, entry_price=entry_price,
                            bet_size=self.bet_size, volume=volume)
        self.open_trades[slug] = t; self._fired.add(slug)
        log.info(f"ALWAYS_DOWN ENTER {slug[-10:]} DOWN @ {entry_price:.3f}")
    ...
    def summary(self):
        ...
        return {'capital':self.capital,'session_start':self.session_start,'n_open':len(self.open_trades),
                ...
                'open_trades':[{'slug':t.slug,'direction':'DOWN','entry_price':round(t.entry_price,4),
                                'volume':round(t.volume,0),'end_ts':t.end_ts,'entered_at':t.entered_at,'url':t.url}
                               for t in self.open_trades.values()],
                'closed_trades':[{'slug':t.slug,'direction':'DOWN','entry_price':round(t.entry_price,4),
