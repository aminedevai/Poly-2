"""
strategy/sniper.py — Volume Spike Sniper

Fires when:
  1. Volume at 30s or 10s is ≥10x the baseline (240s snapshot)
  2. Price has moved ≥12% since the baseline

Direction: follows the spike (momentum, not fade).
Exit: holds to settlement ($1.00 or $0.00 per share).
...
class Sniper:
    ...
    def on_snapshot(self, slug: str, up_price: float, volume: float,
        ...
        vol_thr   = SNIPER["vol_ratio"]
        move_thr  = SNIPER["min_move"]

        # Record baseline at 240s
        ...
        log.info(f"SNIPER @{seconds_before}s {slug[-10:]}  "
                 f"vol={vol_ratio:.1f}x  move={move:+.3f}")

        if vol_ratio < vol_thr:   return
        if abs(move) < move_thr:  return
        ...
        trade = SniperTrade(
            slug=slug, direction=direction, entry_price=entry_price,
            bet_size=bet_size, vol_ratio=vol_ratio, move=move,
        )
        self.open_trades[slug] = trade
        self._fired.add(slug)
        log.info(f"SNIPER ENTER {slug[-10:]} {direction} "
                 f"@ {entry_price:.3f}  vol={vol_ratio:.0f}x")
    ...
    def on_outcome(self, slug: str, outcome: str):
        ...
        self._save(t)
        log.info(f"SNIPER SETTLE {slug[-10:]} {outcome}  "
                 f"{'WIN' if won else 'LOSS'}  profit={t.profit:+.2f}")
